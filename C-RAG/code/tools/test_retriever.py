...
class Retriever:
    ...
    def __init__(self,graph,node_text_keys,embedder_name,cache=True, cache_dir=None):
        logger.info("Initializing retriever")

        #self.use_gpu = use
        self.node_text_keys = node_text_keys
        self.model_name = embedder_name
        self.model = sentence_transformers.SentenceTransformer(embedder_name)
        self.graph = graph
        self.cache = "/home/ubuntu/cot/data/processed_data/maple/Physics/cache-all-mpnet-base-v2.pkl"
        self.cache_dir = "/home/ubuntu/cot/data/processed_data/maple/Physics"
...
if __name__ == '__main__':
    ...
    graph_dir = "/home/ubuntu/cot/data/processed_data/maple/Physics/graph.json"
    node_text_keys = {'paper': ['title'], 'author': ['name'], 'venue': ['name']}

    query ="kelvin helmholtz instabilities in swirling jets"

    graph = json.load(open(graph_dir))
    node_retriever = Retriever(graph, node_text_keys, model_name)
